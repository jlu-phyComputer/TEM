...
from layers1.PatchTST_backbone import PatchTST_backbone
from layers1.PatchTST_layers import series_decomp
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):           # x: [Batch, Input length, Channel]
        ...
        #     x = self.model(x)
        #     x = x.permute(0,2,1)    # x: [Batch, Input length, Channel]
        x_enc = x_enc.permute(0, 2, 1)  # x: [Batch, Channel, Input length]
        x = self.model(x_enc, pi=self.pi, pi_s=self.pi_s)
        x = x.permute(0, 2, 1)  # x: [Batch, Input length, Channel]
        return x