...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None, PE=None, use_DPE=True, use_rot=False,
                init_sim_m=None, pi=None, pis=None):
        # # use DPE
        # x = x.permute(0, 2, 1)
        ...
        if use_DPE:
            # use DPE
            PE = self.pos_embed(x.permute(0, 2, 1))[:, :, :x.shape[-1]].permute(0, 2, 1)
            # x = x + PE
            new_x, attn = self.attention(
                x + pi[0] * PE, x + pi[1] * PE, x + pi[2] * PE,
                attn_mask=attn_mask,
                tau=tau, delta=delta, pi=pi, pis=pis, init_sim_m=init_sim_m, use_rot=use_rot
            )
        else:
            new_x, attn = self.attention(
                x, x, x,
...
class EncoderLayer_cross_tv(nn.Module):
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None, use_DPE=True, use_rot=True,
        ...
        #     x, x, x,
        #     attn_mask=attn_mask,
        #     tau=tau, delta=delta, use_rot=use_rot, init_sim_m=init_sim_m,
        #     pi=pi, pis=pis
        # )
        ...
        if use_DPE:
            ...
            new_x, attn = self.attention(
                x + pi[0] * PE, x + pi[1] * PE, x + pi[2] * PE,
                attn_mask=attn_mask,
                tau=tau, delta=delta, use_rot=use_rot, pi=pi, pis=pis, init_sim_m=init_sim_m,
            )
        else:
            new_x, attn = self.attention(
                x, x, x,
...
class Encoder(nn.Module):
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None, PE=None,
                pi=None, pis=None, init_sim_m=None, use_rot=False):
        # x [B, L, D]
        attns = []
        i = 0
        for attn_layer in self.attn_layers:
            x, attn = attn_layer(x, attn_mask=attn_mask, tau=tau, delta=delta, PE=PE,
                                 init_sim_m=init_sim_m, pi=pi[i * 3: (i + 1) * 3],
                                                   pis=pis[i * 8: (i + 1) * 8], use_DPE=False, use_rot=True)
            attns.append(attn)
            i+=1
...
class Encoder_cross_t_v_duli_KL(nn.Module):
    ...
    def forward(self, x, x_mark_enc, attn_mask=None, tau=None, delta=None,
                pi1=None, pi2=None, pis1=None, pis2=None):
        # x [B, L, D]
        attns = []
        if x_mark_enc is not None:
            x = torch.cat([x, x_mark_enc], -1)
        ...
        init_score_c_var = torch.matmul(enc_out_c_var, enc_out_c_var.
                         permute(0, 2, 1)) / (enc_out_c_var.shape[-1] ** 0.5)
        init_score_c_t = torch.matmul(enc_out_c_time, enc_out_c_time.
                         permute(0, 2, 1)) / (enc_out_c_time.shape[-1] ** 0.5)
        ...
        for attn_layer in self.attn_layers1:
            enc_out_c_var, attn_c_var = attn_layer(enc_out_c_var,
                                                   attn_mask=attn_mask,
                                                   tau=tau, delta=delta,
                                                   use_DPE=False, use_rot=True, init_sim_m=init_score_c_var,
                                                   pi=pi1[i * 3: (i + 1) * 3],
                                                   pis=pis1[i * 8: (i + 1) * 8])
        ...
        for attn_layer in self.attn_layers2:
            enc_out_c_time, attn_c_time = attn_layer(enc_out_c_time,
                                                     attn_mask=attn_mask,
                                                     tau=tau, delta=delta,
                                                     use_DPE=True, use_rot=False, init_sim_m=init_score_c_t,
                                                     pi=pi2[i * 3: (i + 1) * 3],
                                                     pis=pis2[i * 8: (i + 1) * 8])
