...
class FullAttention_with_pos_emb(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None,
        ...
        if self.mask_flag:
            ...
            scores.masked_fill_(attn_mask.mask, -np.inf)
        # scores = scores + pis[0] * init_sim_m.unsqueeze(1)
...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None, pis=None, init_sim_m=None):
        B, L, H, E = queries.shape
        _, S, _, D = values.shape
        scale = self.scale or 1. / sqrt(E)
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None,
        ...
        if use_rot:
            ...
            PE = self.position_embedding(queries)
            # queries = queries + PE
            # keys = keys + PE
            queries = queries + pi[0] * PE
            keys = keys + pi[1] * PE
            values = values + pi[2] * PE
...
class AttentionLayer_with_enhanced_emb(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask,
                tau=None, delta=None, use_rot=True, init_sim_m=None, pi=None, pis=None):
        B, L, _ = queries.shape
        _, S, _ = keys.shape
        H = self.n_heads
        # PE = self.position_embeddings.return_emb()[:queries.shape[1]]
        # PE = get_sinusoid_encoding_table(queries.shape[1], queries.shape[2]).cuda()
        if use_rot:
            # queries = self.position_embeddings(queries)
            # keys = self.position_embeddings(keys)
            # values = self.position_embeddings(values)
            PE = self.position_embeddings.return_emb()[None, :queries.size(1), :]
            # queries = queries + PE
            # keys = keys + PE
            queries = queries + pi[0] * PE
            keys = keys + pi[1] * PE
