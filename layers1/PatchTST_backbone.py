...
from layers1.PatchTST_layers import *
from layers1.RevIN import RevIN
...
class PatchTST_backbone(nn.Module):
    ...
    def forward(self, z, pi=None, pi_s=None):  # z: [bs x nvars x seq_len]
        # norm
        if self.revin:
            z = z.permute(0, 2, 1)
        ...
        # model
        z = self.backbone(z, pi=pi, pis=pi_s)  # z: [bs x nvars x d_model x patch_num]
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    ...
    def forward(self, x, pi=None, pis=None) -> Tensor:  # x: [bs x nvars x patch_len x patch_num]

        n_vars = x.shape[1]
        # Input encoding
        x = x.permute(0, 1, 3, 2)  # x: [bs x nvars x patch_num x patch_len]

        init_score_c_v = torch.matmul(x, x.permute(0, 1, 3, 2))
        ...
        # Encoder
        z = self.encoder(u, pi=pi, pis=pis, init_sim_m=init_score_c_v,
                         PE=self.W_pos)  # z: [bs * nvars x patch_num x d_model]
...
class TSTEncoder(nn.Module):
    ...
    def forward(self, src, key_padding_mask=None, attn_mask=None,
                pi=None, pis=None, init_sim_m=None, PE=None):
        output = src
        scores = None
        if self.res_attention:
            ...
            for mod in self.layers:
                output, scores = mod(output, prev=scores, key_padding_mask=key_padding_mask,
                                     attn_mask=attn_mask, PE=PE,
                                     init_sim_m=init_sim_m, pi=pi[i * 3: (i + 1) * 3],
                                     pis=pis[i * 8: (i + 1) * 8])
                i += 1
            return output
        else:
...
class TSTEncoderLayer(nn.Module):
    ...
    def forward(self, src, prev=None, key_padding_mask=None,
                attn_mask=None, PE=None, init_sim_m=None, pi=None, pis=None):
        ...
        if self.res_attention:
            src2, attn, scores = self.self_attn(src, src, src, prev, key_padding_mask=key_padding_mask,
                                                attn_mask=attn_mask, PE=PE, init_sim_m=init_sim_m, pi=pi, pis=pis)
        else:
...
class _MultiheadAttention(nn.Module):
    ...
    def forward(self, Q, K=None, V=None, prev=None,
                key_padding_mask=None, attn_mask=None, PE=None, init_sim_m=None, pi=None, pis=None):
...
class _ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, prev: Optional[Tensor] = None,
        ...
        if init_sim_m is not None:
            init_sim_m = init_sim_m.reshape(init_sim_m.shape[0] * init_sim_m.shape[1], init_sim_m.shape[2], init_sim_m.shape[3])
            scores = scores + pis.unsqueeze(0).unsqueeze(-1).unsqueeze(-1) * init_sim_m.unsqueeze(1)
        attn_scores = scores * self.scale  # attn_scores : [bs x n_heads x max_q_len x q_len]
